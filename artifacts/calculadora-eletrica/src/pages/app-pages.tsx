...
function StatCard({ label, value, caption, icon: Icon, accent = 'teal' }: { label: string; value: string; caption: string; icon: typeof Activity; accent?: 'teal' | 'amber' | 'blue' }) {
  const colors = {
    teal: 'bg-[#e7f7f3] text-[#25847b]',
    amber: 'bg-[#fff5df] text-[#a66c14]',
    blue: 'bg-[#e9f0fb] text-[#37669d]',
  };
  return (
...
export function DashboardPage({ userName, profile, data }: { userName: string; profile: EnergyProfile | null; data: VoltivaData }) {
  ...
  return (
    <div className="space-y-8">
      <PageIntro eyebrow="Visão geral" title={`Olá, ${userName || 'por aqui'}.`} description="Seu espaço para entender grandezas, acompanhar consumo e tomar decisões elétricas com mais clareza." action={<Link href="/app/calculator" className="focus-ring inline-flex items-center gap-2 rounded-xl bg-[hsl(var(--primary))] px-4 py-3 text-sm font-bold text-white shadow-[0_8px_18px_hsl(var(--primary)/.18)] transition hover:-translate-y-0.5 hover:bg-[#126d91]">Novo cálculo <ArrowRight size={16} /></Link>} />

      <section className="rounded-[26px] border border-[#cfe2df] bg-[#eaf7f4] p-5 sm:p-7">
        <div className="flex flex-col justify-between gap-6 md:flex-row md:items-center">
          <div className="flex items-start gap-4">
            <span className="grid size-12 shrink-0 place-items-center rounded-2xl bg-white text-[#278b82] shadow-sm"><Zap size={22} fill="currentColor" /></span>
            <div>
              <p className="text-xs font-bold uppercase tracking-[0.13em] text-[#287b76]">Seu ponto de partida</p>
              <h2 className="mt-1 font-display text-2xl font-semibold tracking-[-0.04em] text-[#1f6062]">{profile ? goalLabels[profile.goal] : 'Complete seu perfil energético'}</h2>
              <p className="mt-2 max-w-xl text-sm leading-6 text-[#4d7f7c]">{profile ? `Você marcou ${profile.interests.length} ${profile.interests.length === 1 ? 'interesse' : 'interesses'} para acompanhar na Voltiva.` : 'Responda três perguntas rápidas para ajustar sua experiência.'}</p>
            </div>
          </div>
          <Link href={profile ? '/app/settings' : '/app/dashboard'} className="focus-ring inline-flex shrink-0 items-center justify-center gap-2 rounded-xl border border-[#add5cf] bg-white px-4 py-2.5 text-sm font-bold text-[#287873] transition hover:border-[#58aca4]">{profile ? 'Ver perfil' : 'Começar perfil'} <ArrowRight size={15} /></Link>
        </div>
      </section>
      ...
      <section className="grid gap-5 lg:grid-cols-[1.15fr_0.85fr]">
        ...
        <div className="rounded-2xl border border-[hsl(var(--border))] bg-white/75 p-5 sm:p-7">
          ...
          <div className="mt-4 space-y-2">
            ...
            ].map(({ href, label, icon: Icon, text }) => <Link key={href} href={href} className="group flex items-center gap-3 rounded-xl border border-transparent p-3 transition hover:border-[#c8e3df] hover:bg-[#f4fbf9]"><span className="grid size-9 place-items-center rounded-lg bg-[#edf7f5] text-[#318d85]"><Icon size={16} /></span><span className="min-w-0 flex-1"><strong className="block text-sm text-[hsl(var(--foreground))]">{label}</strong><span className="text-xs text-[hsl(var(--muted-foreground))]">{text}</span></span><ArrowRight size={15} className="text-[#9ab3b2] transition group-hover:translate-x-1 group-hover:text-[#318d85]" /></Link>)}
          </div>
        </div>
      </section>
    </div>
    ...
    <div className="space-y-8">
      ...
      <section className="grid gap-5 lg:grid-cols-[0.82fr_1.18fr]">
        <div className="soft-shadow rounded-2xl border border-[hsl(var(--card-border))] bg-white p-5 sm:p-7">
          ...
          <button onClick={submit} className="focus-ring mt-6 inline-flex w-full items-center justify-center gap-2 rounded-xl bg-[hsl(var(--primary))] px-4 py-3 text-sm font-bold text-white transition hover:bg-[#126d91]"><Plus size={17} /> Adicionar equipamento</button>
        </div>
        <div className="soft-shadow rounded-2xl border border-[hsl(var(--card-border))] bg-white p-5 sm:p-7">
          <div className="flex items-end justify-between gap-4"><div><p className="text-xs font-bold uppercase tracking-[0.13em] text-[hsl(var(--muted-foreground))]">Estimativa mensal</p><h2 className="mt-2 font-data text-3xl font-medium tracking-[-0.06em]">{formatNumber(total)} kWh</h2></div><span className="grid size-11 place-items-center rounded-2xl bg-[#e7f7f3] text-[#25847b]"><Activity size={21} /></span></div>
...
export function SavingsPage({ data }: { data: VoltivaData }) {
  ...
  return (
    <div className="space-y-8">
      ...
      <section className="soft-shadow rounded-2xl border border-[hsl(var(--card-border))] bg-white p-5 sm:p-7"><div className="flex items-center justify-between gap-4"><div><p className="text-xs font-bold uppercase tracking-[0.13em] text-[hsl(var(--muted-foreground))]">Acompanhamento</p><h2 className="mt-2 font-display text-xl font-semibold tracking-[-0.035em]">Uso frente à sua meta</h2></div><span className="font-data text-sm font-medium text-[#2c817b]">{formatNumber(progress)}%</span></div><div className="mt-6 h-3 overflow-hidden rounded-full bg-[#e6efed]"><div className="h-full rounded-full bg-[#38aaa0] transition-all" style={{ width: `${progress}%` }} /></div><p className="mt-3 text-sm leading-6 text-[hsl(var(--muted-foreground))]">{total ? 'Use esta referência para decidir onde investigar primeiro. Uma redução real deve ser confirmada com a sua medição ou fatura.' : 'Cadastre equipamentos na área de consumo para começar a acompanhar uma base real.'}</p></section>
      <section className="grid gap-4 md:grid-cols-2">{['Desligue cargas em espera quando não forem necessárias.', 'Compare o tempo de uso antes de trocar um equipamento.', 'Registre a potência nominal do equipamento, não uma estimativa.', 'Valide qualquer economia com uma leitura ou fatura real.'].map((tip, index) => <div key={tip} className="flex gap-3 rounded-2xl border border-[#d6e5e2] bg-[#f1f8f6] p-5"><span className="grid size-8 shrink-0 place-items-center rounded-lg bg-white font-data text-xs font-bold text-[#2c827b]">{String(index + 1).padStart(2, '0')}</span><p className="text-sm leading-6 text-[#4e7774]">{tip}</p></div>)}</section>
    </div>
  );
}
...
export function ReportsPage({ calculations, devices }: { calculations: CalculationRecord[]; devices: EnergyDevice[] }) {
  ...
  return (
    <div className="space-y-8">
      <PageIntro eyebrow="Saída de dados" title="Relatórios" description="Exporte um resumo dos seus registros para continuar a análise fora da Voltiva." action={<button onClick={exportCsv} disabled={!calculations.length} className="focus-ring inline-flex items-center gap-2 rounded-xl bg-[hsl(var(--primary))] px-4 py-3 text-sm font-bold text-white transition hover:bg-[#126d91] disabled:cursor-not-allowed disabled:opacity-50"><Download size={16} /> Exportar CSV</button>} />
      ...
      <section className="grid gap-5 md:grid-cols-2"><div className="soft-shadow rounded-2xl border border-[hsl(var(--card-border))] bg-white p-6"><div className="flex items-center gap-3"><span className="grid size-10 place-items-center rounded-xl bg-[#e7f7f3] text-[#25847b]"><BarChart3 size={18} /></span><div><p className="text-xs font-bold uppercase tracking-[0.13em] text-[hsl(var(--muted-foreground))]">Resumo</p><h2 className="mt-1 font-display text-xl font-semibold">{calculations.length} cálculos salvos</h2></div></div><p className="mt-5 text-sm leading-6 text-[hsl(var(--muted-foreground))]">O arquivo contém os resultados e as fórmulas aplicadas, sem alterar os dados originais.</p></div><div className="soft-shadow rounded-2xl border border-[hsl(var(--card-border))] bg-white p-6"><div className="flex items-center gap-3"><span className="grid size-10 place-items-center rounded-xl bg-[#fff5df] text-[#a66c14]"><Gauge size={18} /></span><div><p className="text-xs font-bold uppercase tracking-[0.13em] text-[hsl(var(--muted-foreground))]">Consumo</p><h2 className="mt-1 font-display text-xl font-semibold">{devices.length} equipamentos</h2></div></div><p className="mt-5 text-sm leading-6 text-[hsl(var(--muted-foreground))]">Os equipamentos cadastrados ficam disponíveis na área de consumo para revisão.</p></div></section>
    </div>
  );
}
  ...
  return (
    <div className="space-y-8">
      ...
      <section className="grid gap-5 lg:grid-cols-2">
        ...
        <div className="soft-shadow rounded-2xl border border-[hsl(var(--card-border))] bg-white p-6 sm:p-7"><div className="flex items-center gap-3"><span className="grid size-10 place-items-center rounded-xl bg-[#fff5df] text-[#a66c14]"><Gauge size={18} /></span><div><p className="text-xs font-bold uppercase tracking-[0.13em] text-[hsl(var(--muted-foreground))]">Preferência</p><h2 className="mt-1 font-display text-xl font-semibold">Meta mensal de consumo</h2></div></div><div className="mt-5 flex items-center gap-3"><input value={goal} onChange={(event) => setGoal(event.target.value)} inputMode="decimal" className="focus-ring h-12 min-w-0 flex-1 rounded-xl border border-[hsl(var(--input))] px-4 font-data text-sm outline-none focus:border-[#39aaa6]" /><span className="font-data text-xs text-[hsl(var(--muted-foreground))]">kWh</span><button onClick={saveGoal} className="focus-ring rounded-xl bg-[hsl(var(--primary))] px-4 py-3 text-sm font-bold text-white transition hover:bg-[#126d91]"><Check size={16} /></button></div><p className="mt-3 text-xs leading-5 text-[hsl(var(--muted-foreground))]">Usada apenas para comparação visual na página Economia.</p></div>
      </section>
...
function Field({ label, value, onChange, placeholder, suffix, inputMode = 'text' }: { label: string; value: string; onChange: (value: string) => void; placeholder: string; suffix?: string; inputMode?: 'text' | 'decimal' | 'numeric' }) {
  return <label className="block"><span className="mb-2 block text-sm font-semibold">{label}</span><div className="flex h-12 items-center rounded-xl border border-[hsl(var(--input))] bg-white focus-within:border-[#39aaa6] focus-within:ring-4 focus-within:ring-[#39aaa61f]"><input value={value} onChange={(event) => onChange(event.target.value)} placeholder={placeholder} inputMode={inputMode} className="focus-ring h-full min-w-0 flex-1 rounded-xl bg-transparent px-4 text-sm font-semibold outline-none placeholder:text-[#bdc8c9]" />{suffix && <span className="pr-4 font-data text-xs text-[hsl(var(--muted-foreground))]">{suffix}</span>}</div></label>;
}

function CalculationRow({ record, detailed = false }: { record: CalculationRecord; detailed?: boolean }) {
  return <div className="flex flex-col gap-3 py-4 first:pt-0 last:pb-0 sm:flex-row sm:items-center sm:justify-between"><div className="flex items-center gap-3"><span className="grid size-9 place-items-center rounded-xl bg-[#edf7f5] font-data text-xs font-bold text-[#318d85]">{calculationMeta[record.type].symbol}</span><div><p className="text-sm font-bold">{calculationMeta[record.type].label}</p><p className="mt-0.5 text-xs text-[hsl(var(--muted-foreground))]">{detailed ? record.formula : 'Fórmula de Ohm'} · {new Date(record.createdAt).toLocaleDateString('pt-BR')}</p></div></div><div className="flex items-center justify-between gap-4 sm:justify-end"><span className="font-data text-base font-medium text-[#267b78]">{formatNumber(record.result)} {record.unit}</span>{detailed && <span className="hidden rounded-full bg-[#eef8f5] px-2 py-1 text-[10px] font-bold text-[#287b76] sm:inline">Salvo</span>}</div></div>;
}

function DeviceRow({ device, onRemove }: { device: EnergyDevice; onRemove: () => void }) {
  return <div className="flex items-center gap-3 rounded-xl border border-[hsl(var(--border))] bg-[#fbfdfd] p-3"><span className="grid size-9 place-items-center rounded-lg bg-[#fff5df] text-[#a66c14]"><Zap size={16} /></span><div className="min-w-0 flex-1"><p className="truncate text-sm font-bold">{device.name}</p><p className="mt-0.5 text-xs text-[hsl(var(--muted-foreground))]">{device.watts} W · {device.hoursPerDay} h/dia · {formatNumber(deviceMonthlyKwh(device))} kWh/mês</p></div><button onClick={onRemove} className="focus-ring rounded-lg p-2 text-[#b47b78] transition hover:bg-[#fff0ee] hover:text-[#b04f4c]" aria-label={`Remover ${device.name}`}><Trash2 size={15} /></button></div>;
}
