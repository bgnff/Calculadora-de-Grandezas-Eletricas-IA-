  ...
  return (
    <div className="space-y-8">
      ...
      <section className="grid gap-5 lg:grid-cols-2">
        ...
        <div className="soft-shadow rounded-2xl border border-[hsl(var(--card-border))] bg-white p-6 sm:p-7"><div className="flex items-center gap-3"><span className="grid size-10 place-items-center rounded-xl bg-[#eaf7ff] text-[#16658e]"><Gauge size={18} /></span><div><p className="text-xs font-bold uppercase tracking-[0.13em] text-[hsl(var(--muted-foreground))]">Preferência</p><h2 className="mt-1 font-display text-xl font-semibold">Meta mensal de consumo</h2></div></div><div className="mt-5 flex items-center gap-3"><input value={goal} onChange={(event) => setGoal(event.target.value)} inputMode="decimal" className="focus-ring h-12 min-w-0 flex-1 rounded-lg border border-[hsl(var(--input))] px-4 font-data text-sm outline-none focus:border-[#006bff]" /><span className="font-data text-xs text-[hsl(var(--muted-foreground))]">kWh</span><button onClick={saveGoal} className="focus-ring rounded-lg bg-[hsl(var(--primary))] px-4 py-3 text-sm font-bold text-white transition hover:bg-[#004eba]"><Check size={16} /></button></div><p className="mt-3 text-xs leading-5 text-[hsl(var(--muted-foreground))]">Usada apenas para comparação visual na página Economia.</p></div>
      </section>
...
function DeviceRow({ device, onRemove }: { device: EnergyDevice; onRemove: () => void }) {
  return <div className="flex items-center gap-3 rounded-lg border border-[hsl(var(--border))] bg-[#fbfdff] p-3"><span className="grid size-9 place-items-center rounded-lg bg-[#eaf7ff] text-[#16658e]"><Zap size={16} /></span><div className="min-w-0 flex-1"><p className="truncate text-sm font-bold">{device.name}</p><p className="mt-0.5 text-xs text-[hsl(var(--muted-foreground))]">{device.watts} W · {device.hoursPerDay} h/dia · {formatNumber(deviceMonthlyKwh(device))} kWh/mês</p></div><button onClick={onRemove} className="focus-ring rounded-lg p-2 text-[#b47b78] transition hover:bg-[#fff0ee] hover:text-[#b04f4c]" aria-label={`Remover ${device.name}`}><Trash2 size={15} /></button></div>;
}
